...
class MViTv1(torch.nn.Module):
    ...
    def __init__(self,
        ...
        #
        self.layer_mvit_v1 = mvit_v1_b(weights = MViT_V1_B_Weights.KINETICS400_V1)
        #
...
#
##
def run_mvit_v1(data_train_set: VideoDataset,
    ...
    #
    ##
    var_macs, var_params = get_model_complexity_info(MViTv1(var_x_shape, var_y_shape), 
    ...
    for var_r in range(var_repeat):
        ...
        #
        model_mvit_v1 = MViTv1(var_x_shape, var_y_shape).to(device)
        #
        if var_weight is not None:  model_mvit_v1.load_state_dict(torch.load(var_weight))
        #
        optimizer = torch.optim.Adam(model_mvit_v1.parameters(), 
                                     lr = preset["nn"]["lr"],
                                     weight_decay = 0)
        #
        ...
        ## ---------------------------------------- Train -----------------------------------------
        #
        var_best_weight = train(model = model_mvit_v1, 
                                optimizer = optimizer, 
                                loss = loss, 
        ...
        ## ---------------------------------------- Test ------------------------------------------
        #
        model_mvit_v1.load_state_dict(var_best_weight)
        #
        result_acc, result_dict, _ = test(model_mvit_v1, 
                                          loss, 
                                          data_test_set, 
                                          preset["nn"]["threshold"], 
