"""
[file]          swin_t.py
[description]   implement and evaluate video-based model Swin-T
"""
#
##
import time
...
class SwinT(torch.nn.Module):
    ...
    def __init__(self,
        ...
        #
        self.layer_swin_t = swin3d_t(weights = Swin3D_T_Weights.KINETICS400_V1)
        #
...
#
##
def run_swin_t(data_train_set: VideoDataset,
               data_test_set: VideoDataset,
               var_repeat: int,
               var_weight = None):
    """
    [description]
    : run video-based model Swin-T
    [parameter]
    ...
    #
    ##
    var_macs, var_params = get_model_complexity_info(SwinT(var_y_shape), 
    ...
    for var_r in range(var_repeat):
        ...
        #
        model_swin_t = SwinT(var_y_shape).to(device)
        #
        if var_weight is not None:  model_swin_t.load_state_dict(torch.load(var_weight))
        #
        optimizer = torch.optim.Adam(model_swin_t.parameters(), 
                                     lr = preset["nn"]["lr"],
                                     weight_decay = 0)
        #
        ...
        ## ---------------------------------------- Train -----------------------------------------
        #
        var_best_weight = train(model = model_swin_t, 
                                optimizer = optimizer, 
                                loss = loss, 
        ...
        ## ---------------------------------------- Test ------------------------------------------
        #
        model_swin_t.load_state_dict(var_best_weight)
        #
        result_acc, result_dict, _ = test(model_swin_t, 
                                          loss, 
                                          data_test_set, 
                                          preset["nn"]["threshold"], 
